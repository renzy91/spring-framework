...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			// <2> 验证和准备覆盖方法
			/**
			 * 只是对 methodOverrides 做校: 若要创建的类中没有 methodOverrides 要覆盖的方法名称，则抛出异常；
			 * 若只有一个则在 methodOverride 进行标记(主要防止要创建的类中有重载时，判断具体替换方法)
			 */
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// 后置处理修改 BeanDefinition @Autowired
		...
		try {
			...
			// <6> 调用初始化方法 实现了InitializingBean 或 有init-method
		...
		if (earlySingletonExposure) {
			 ...
			 *
			 * 非循环依赖情况还没有到调用addSingleton方法的地方，singletonObjects中没有当前bean；
			 * 若出现了循环依赖情况earlySingletonObjects会有值
			 */
			Object earlySingletonReference = getSingleton(beanName, false);
			if (earlySingletonReference != null) {
				// 如果 exposedObject 没有在初始化方法中被改变，也就是没有被增强
				// initializeBean 中调用BeanPostProcessor可以改变bean
				...
				// 检测依赖
					...
					/**
					 * exposedObject == bean 证明initializeBean()已经改变了bean，依赖此bean的bean拿到的是
					 */
					// 所有依赖此 bean 的 beanName
					String[] dependentBeans = getDependentBeans(beanName);
					Set<String> actualDependentBeans = new LinkedHashSet<>(dependentBeans.length);
					for (String dependentBean : dependentBeans) {

						// 创建完成放入 actualDependentBeans
						if (!removeSingletonIfCreatedForTypeCheckOnly(dependentBean)) {
							actualDependentBeans.add(dependentBean);
						}
					}
					// actualDependentBeans 非空抛异常
		...
		// <8> 根据 scope 注册 bean, 如果配置了destroy-method
		// 注册销毁时回调方法
		try {
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		//首先会检查是否实现了 InitializingBean, 如果是的话需要调用 afterPropertiesSet 方法
		...
		// 如果指定了init-method并且不是实现InitializingBean的afterPropertiesSet方法
